...
def _stream_and_verify(broker, tunnel, agent, port: int, count: int, label: str) -> None:
    """Push `count` generated bytes at `port`, then check the sink's digest.

    Sustained transfers are where backpressure bugs surface: the queues stay
    partly drained for minutes, which is exactly the state that used to corrupt
    the stream and wedge the TLS shutdown.
    """
    ...
    print(
        f"\nlarge_stream mode={label} bytes={count} seconds={elapsed:.3f} "
...
def _large_byte_count() -> int:
    return int(os.getenv("TUNNELMATE_LARGE_BYTES", "0"))


@pytest.mark.extended
@pytest.mark.skipif(
    not _large_byte_count(),
    reason="set TUNNELMATE_LARGE_BYTES=6442450944 for the extended stream test",
)
def test_large_open_tcp_stream_without_spooling(broker) -> None:
    """Stream >5 GiB through an open tunnel, hash-verified, bounded memory."""
    count = _large_byte_count()
    ...
    _stream_and_verify(broker, tunnel, agent, tunnel["public_port"], count, "open")
    server.shutdown()
    server.server_close()
...
def test_large_closed_tcp_stream_without_spooling(broker) -> None:
    """The same volume through the authenticated closed-mode peer relay."""
    count = _large_byte_count()
    server = threaded_tcp_server(HashSinkHandler)
    tunnel = broker.create_tunnel("large-closed-test", proto="tcp", closed=True)
    agent = start_agent(broker, tunnel, server.server_address[1], "tcp")
    peer_port = free_tcp_port()
    start_peer(broker, tunnel, tunnel["shared_token"], peer_port, "tcp")
    _stream_and_verify(broker, tunnel, agent, peer_port, count, "closed")
    server.shutdown()
    server.server_close()
